...
class Uploader extends Controller {
  async uploadImg() {
    ...
    const nameid = ctx.helper.uuid.v4().replace(/-/g, '')
    // path.extname(uploadStream.filename).toLocaleLowerCase可以获取到文件后缀名，比如.jpg .png
    const filename =
      nameid + path.extname(uploadStream.filename).toLocaleLowerCase()
  ...
  async uploadImgs() {
    ...
    while ((part = await parts()) != null) {
      if (part.length) {
        ...
        const filename =
          nameid + path.extname(part.filename).toLocaleLowerCase()
        // 文件存在 upload/img 文件夹下
        const target = path.join(config.baseDir, 'upload/img', filename)
        // 生成一个文件写入 文件流
        const writeStream = fs.createWriteStream(target)
